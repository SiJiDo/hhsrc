...
app = Celery('hhsrc.httpx', broker=broker, backend=backend, )
...
def run(target):
    ...
    if DEBUG == 'True':
        ...
        command = ['./httpx_mac', '-l', target_file, '-cdn', '-follow-host-redirects' , '-json', '-o', out_file_name]
    else:
        command = ['./httpx', '-l', target_file, '-cdn', '-follow-host-redirects' , '-json', '-o', out_file_name]
    result = []
    #整体抛出异常，以防httpx自身报错
    try:
